...
public class AlunoDAO extends StandardDAO<Aluno>{
	public boolean insert(Aluno e) {
		...
		sql = "insert into tabela_endereco (rua,bairro,numero,mat_aluno) values (?,?,?,?);";
		try {
			PreparedStatement ps = getConnection().prepareStatement(sql);
	...
	public boolean delete(Aluno e) {
		...
		sql = "delete from tabela_endereco where mat_aluno=?;";
		try {
			PreparedStatement ps = getConnection().prepareStatement(sql);
			ps.setString(1, e.getMatricula());
			ps.execute();
	...
	public boolean alter(Aluno e) {
		String sql = "update tabela_aluno set matricula=?,nome=? where matricula=?;";
		try {
			PreparedStatement ps = getConnection().prepareStatement(sql);
			ps.setString(1, e.getMatricula());
			ps.setString(2, e.getNome());
		...
		sql = "update tabela_endereco set rua=?, bairro=?, numero=? where mat_aluno=?;";
		try {
			PreparedStatement ps = getConnection().prepareStatement(sql);
			ps.setString(1, e.getEndereco().getRua());
			ps.setString(2, e.getEndereco().getBairro());
